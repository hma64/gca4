...
window.editProduct = (id) => {
  ...
  adminForm.innerHTML = `
    ...
    <div class="form-group">
      <label>Formes / Dimensions <small style="font-weight:400;color:#999;">(optionnel — ex: Rond Ø 120cm, Rond Ø 150cm)</small></label>
      <input type="text" name="formes" value="${(p.formes || []).join(', ')}" placeholder="Laissez vide si non applicable">
    </div>
    ...
    <div class="order-item">
      ...
      <div class="order-item-details">
        ...
        <div class="order-item-specs">
          Couleur: ${item.color}${item.forme && item.forme !== '-' ? ` | Forme: ${item.forme}` : ''}
        </div>
...
adminForm.onsubmit = async (e) => {
  ...
  if (currentTab === 'products') {
    ...
    data.formes = formData.get('formes').split(',').map(s => s.trim()).filter(Boolean);
