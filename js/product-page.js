...
const formeOptions = document.getElementById("formeOptions");
  ...
  values.forEach((value, idx) => {
    ...
    if (type === "color") {
      ...
      btn.addEventListener("click", () => {
        ...
        if (alreadySelected) {
          btn.classList.remove("selected");
          if (type === "forme") selectedForme = "";
          updateStockMessage();
          return;
        }
        container.querySelectorAll("button").forEach(b => b.classList.remove("selected"));
        btn.classList.add("selected");
        if (type === "forme") selectedForme = value;
        selectionError.textContent = "";
        updateStockMessage();
      });
    }
...
function openProductDetails(product) {
  ...
  if (formeGroup) formeGroup.style.display = product.formes && product.formes.length ? "" : "none";
  if (formeOptions) renderOptionButtons(formeOptions, product.formes || [], "forme");
  syncDetailWishBtn();
...
function addProductToCart(product, color, forme) {
  const priceData = getPriceData(product);
  const cart = getCart();
  cart.push({
...
addToCartBtn.addEventListener("click", () => {
  ...
  const needForme = currentProduct.formes && currentProduct.formes.length > 0;

  if ((needColor && !selectedColor) || (needForme && !selectedForme)) {
  ...
  addProductToCart(currentProduct, selectedColor, selectedForme);
  selectionError.textContent = "";
